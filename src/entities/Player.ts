...
const TURN_SPEED = 0.04;  // radians per frame for yaw rotation
...
export class Player {
    ...
    private _handleTurning(): void {
        const turn = this._input.axis('KeyA', 'KeyD') +
            this._input.axis('ArrowLeft', 'ArrowRight');
        if (turn === 0) return;

        // axis = outward normal ("local up"), angle = turn direction × speed
        _scratchQuat.setFromAxisAngle(this._state.surfaceNormal, turn * TURN_SPEED);
        this._state.forward.applyQuaternion(_scratchQuat);

        // Re-orthogonalise: remove any component along the surface normal
        // that floating-point arithmetic introduced.
        this._state.forward
            .addScaledVector(
    ...
    private _handleMovement(): void {
        ...
        // right = forward × surfaceNormal  (perpendicular, in tangent plane)
        _scratchVec
            .crossVectors(this._state.forward, this._state.surfaceNormal)
            .normalize();

        // Rotate the surface normal around the right vector by ±MOVE_SPEED.
        // A positive move steps toward forward; negative steps backward.
        _scratchQuat.setFromAxisAngle(_scratchVec, move * MOVE_SPEED);
        this._state.surfaceNormal.applyQuaternion(_scratchQuat).normalize();

        // The forward vector must travel with the normal (same rotation)
        // to stay tangent to the sphere after the normal has moved.
        this._state.forward.applyQuaternion(_scratchQuat);
        this._state.forward
            .addScaledVector(
                this._state.surfaceNormal,
    ...
    private _updateCamera(): void {
        ...
        const localOffset = new THREE.Vector3(0, CAM_HEIGHT, CAM_DISTANCE);
        const worldOffset = localOffset.applyQuaternion(this._group.quaternion);
        const targetPos = this._group.position.clone().add(worldOffset);
        ...
        const lookTarget = this._group.position.clone()
