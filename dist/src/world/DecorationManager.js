...
export class DecorationManager {
    ...
    static _WORLD_UP = new THREE.Vector3(0, 1, 0);
    constructor(scene) {
     ...
     */
    _placeOnSurface(obj, latDeg, lonDeg) {
        ...
        //   z = R·cos(lat)·sin(lon)
        const surfacePos = new THREE.Vector3(PLANET_RADIUS * Math.cos(lat) * Math.cos(lon), PLANET_RADIUS * Math.sin(lat), PLANET_RADIUS * Math.cos(lat) * Math.sin(lon));
    ...
    _buildTree() {
        ...
        const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.09, 0.6, 6), new THREE.MeshStandardMaterial({ color: 0x6b3c11, roughness: 0.9 }));
        trunk.castShadow = true;
        const foliage = new THREE.Mesh(new THREE.ConeGeometry(0.35, 0.9, 6), new THREE.MeshStandardMaterial({ color: 0x1a7a2a, roughness: 0.8 }));
    ...
    _buildRock() {
        const rock = new THREE.Mesh(new THREE.DodecahedronGeometry(0.22 + Math.random() * 0.15, 0), new THREE.MeshStandardMaterial({ color: 0x888888, roughness: 0.95 }));
